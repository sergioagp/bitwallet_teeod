...
static TEEC_Result ta_bitcoin_cmd_generate_new_masterkey(TEEC_Session sess, uint32_t pin, uint32_t cmd_id, uint32_t err_origin){
	...
	op.params[0].value.a = pin;
	op.params[0].value.b = 0; // 0---unchecked, 1---correct pin, 2---incorrect pin
	TEEC_SharedMemory tmp_sh;
  tmp_sh.size = 128;
  TEEC_AllocateSharedMemory(NULL, &tmp_sh);
  op.params[1].memref.parent = &tmp_sh;
    ...
    {
      printf("%c", *((char*) op.params[1].memref.parent->buffer+i));
    }
    printf("\r\n");
	}
  TEEC_ReleaseSharedMemory(&tmp_sh);
	return TEEC_SUCCESS;
}
...
static TEEC_Result ta_bitcoin_cmd_mnemonic_to_masterkey(TEEC_Session sess, uint32_t pin, uint32_t cmd_id, uint32_t err_origin, char* mnemonic){
	...
	op.params[0].value.b = 0; // 0---unchecked, 1---correct pin, 2---incorrect pin
  TEEC_SharedMemory tmp_sh;
  tmp_sh.size = strlen(mnemonic);
  TEEC_AllocateSharedMemory(NULL, &tmp_sh);
  op.params[1].memref.parent = &tmp_sh;
	op.params[1].memref.size = strlen(mnemonic);

  for (size_t i = 0; i < op.params[1].memref.size; i++) {
    *((char*) op.params[1].memref.parent->buffer+i) = mnemonic[i];
  }
...
static TEEC_Result ta_bitcoin_cmd_issue_transactions(TEEC_Session sess, uint32_t pin, uint32_t cmd_id, uint32_t err_origin, uint32_t account_id){
  ...
  TEEC_SharedMemory tmp_sh1, tmp_sh2;
  
  tmp_sh1.size = get_txbuf_next(buf);
  TEEC_AllocateSharedMemory(NULL, &tmp_sh1);
  op.params[1].memref.parent = &tmp_sh1;
	op.params[1].memref.size = get_txbuf_next(buf);
  for (size_t i = 0; i < op.params[1].memref.size; i++) {
    *((char*) op.params[1].memref.parent->buffer+i) = *((char*) get_txbuf_data(buf)+i);
  }

  tmp_sh2.size = 72;
  TEEC_AllocateSharedMemory(NULL, &tmp_sh2);
  op.params[2].memref.parent = &tmp_sh2;
	op.params[2].memref.size = 72;
  ...
  for (size_t i = 0; i < op.params[2].memref.size; i++)
  {
    printf("%c", *((char*) op.params[2].memref.parent->buffer+i));
  }
  printf("\r\n");

  TEEC_ReleaseSharedMemory(&tmp_sh1);
  TEEC_ReleaseSharedMemory(&tmp_sh2);
	return res;
}
...
static TEEC_Result ta_bitcoin_cmd_get_bitcoin_address(TEEC_Session sess, uint32_t pin, uint32_t cmd_id, uint32_t err_origin, uint32_t account_id){
  ...
  tmp_sh.size = 25;
  TEEC_AllocateSharedMemory(NULL, &tmp_sh);
  op.params[2].memref.parent = &tmp_sh;
	op.params[2].memref.size = 25;
